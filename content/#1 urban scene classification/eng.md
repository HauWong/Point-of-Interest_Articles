...
- **2020 - Heuristic sample learning for complex urban scenes: Application to urban functional-zone mapping with VHR images and POI data - Xiuyuan Zhang**
	- **内容**：现有的应用高分辨率遥感影像的功能区识别研究多聚焦于特征表达和分类方法上，而忽略了功能区的采样。由于功能区是一种复杂的城市景观，由不同类型的土地覆盖构成，具有高度抽象的类别，为此，本文提出一种新的采样方法，即启发式样本学习（Heuristic Sample Learning，HSL）。
	- **方法**：
		1. 启发式样本学习源自主动学习，即先通过专家标注样本，在以此样本训练分类器，经过分类器分类后，专家再对结果中的混淆样本进行标注，重复此过程至到不再出现混淆样本，而启发式采样则在上述过程中加入启发式标记模块，辅助专家进行标注；
		2. 启发式样本学习应用于功能区分类，首先利用稀疏主题模型选择具有代表性的功能区，然后采用深度森林方法选择混淆功能区，最后采用中餐厅过程进行功能区标注。